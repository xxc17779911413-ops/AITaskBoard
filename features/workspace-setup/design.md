...
**R8 保留分支时必须保留 `attrs.branch`（N1 回归点）**：清理只在分支**确实已不存在**
（被删或本就已删）时才清空 `attrs.branch`。`--keep-branch` / `removeBranch:false`，
以及「未并入基线而保守保留」的分支，都必须保留属性值——否则工具侧后续 cleanup 取不到分支名，
`unit prompt` 也会回退到模板重算，报出与实际保留分支不同的名字（开发者可能据此去错分支开工）。
